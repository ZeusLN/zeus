...
export default class TransactionView extends React.Component<TransactionProps> {
    render() {
        const { NodeInfoStore, UnitsStore, SettingsStore, navigation } = this.props;
        ...
        return (
            <ScrollView style={theme === 'dark' ? styles.darkThemeStyle : styles.lightThemeStyle}>
                ...
                <View style={styles.content}>
                    <Text style={theme === 'dark' ? styles.labelDark : styles.label}>Total Fees:</Text>
                    <TouchableOpacity onPress={() => changeUnits()}>
                        <Text style={theme === 'dark' ? styles.valueDark : styles.value}>{units && getAmount(total_fees || 0)}</Text>
                    </TouchableOpacity>

                    <Text style={theme === 'dark' ? styles.labelDark : styles.label}>Transaction Hash:</Text>
                    <TouchableOpacity onPress={() => UrlUtils.goToBlockExplorerTXID(tx_hash, testnet)}>
                        <Text style={styles.valueWithLink}>{tx_hash}</Text>
                    </TouchableOpacity>

                    {block_hash && <Text style={theme === 'dark' ? styles.labelDark : styles.label}>Block Hash:</Text>}
                    ...
                    {block_height && <Text style={theme === 'dark' ? styles.labelDark : styles.label}>Block Height:</Text>}
                    ...
                    <Text style={theme === 'dark' ? styles.labelDark : styles.label}>Number of Confirmations:</Text>
                    <Text style={{ ...styles.value, color: num_confirmations > 0 ? 'green' : 'red' }}>{num_confirmations || 0}</Text>

                    <Text style={theme === 'dark' ? styles.labelDark : styles.label}>Timestamp:</Text>
                    <Text style={theme === 'dark' ? styles.valueDark : styles.value}>{date.toString()}</Text>

                    {dest_addresses && <Text style={theme === 'dark' ? styles.labelDark : styles.label}>{dest_addresses.length > 1 ? 'Destination Addresses:' : 'Destination Address:'}</Text>}
