...
> {
    constructor(props: RebalanceProps) {
        ...
        this.state = {
            ...
            totalBalance: 0,
            balanceSliderValue: REBALANCE_CONSTANTS.DEFAULT_SLIDER_VALUE // Start at middle (balanced)
        };
    }
    ...
    private handleChannelSelect = (channel: Channel, type: ChannelType) => {
        ...
        this.setState(stateUpdates as RebalanceState, () => {
    ...
    initializeBalanceSlider = () => {
        ...
        if (selectedSourceChannel && selectedDestinationChannel) {
            ...
            this.setState({
                ...
                totalBalance: total,
                balanceSliderValue: 50
            });
        }
    };

    onBalanceSliderChange = (value: number) => {
        const { totalBalance, originalDestinationBalance } = this.state;

        // Value is 0-100, convert to percentage
        const percentage = value / 100;

        // Calculate new balances (when slider is 0, source gets all, when 100, dest gets all)
        const newDestBalance = Math.round(totalBalance * percentage);
        const newSourceBalance = totalBalance - newDestBalance;

        // Calculate rebalance amount (difference from original)
        const rebalanceAmount = Math.abs(
            newDestBalance - originalDestinationBalance
        );

        this.setState(
            {
                balanceSliderValue: value,
                adjustedSourceBalance: newSourceBalance,
                adjustedDestinationBalance: newDestBalance,
                rebalanceAmount
            },
            () => {
                this.updateProjectedBalances();
    ...
    private handleFailedPayment = (payment: any) => {
        ...
        Alert.alert(localeString('views.Rebalance.failed.title'), errorMsg);
    };
    ...
    private renderChannelItem = (channel: Channel): React.ReactElement => (
        <View
            style={[
                styles.channelInfo,
    ...
    private renderChannelSelectionSection = (
        ...
        return (
            <View style={styles.section}>
                ...
                {selectedChannel && this.renderChannelItem(selectedChannel)}
            </View>
        );
    };
    ...
    private renderBalanceAdjustmentSection = (): React.ReactElement => {
        ...
        return (
            <View style={styles.section}>
                ...
                {/* Current adjusted balances display */}
                <View
                    style={[
                        styles.balanceAdjustmentContainer,
                        { backgroundColor: themeColor('secondary') }
                    ]}
                >
                    <View style={styles.channelBalanceRow}>
                        ...
                        <Amount
                            sats={this.state.adjustedSourceBalance || 0}
                            sensitive
                        />
                    </View>
                    ...
                    <View style={styles.channelBalanceRow}>
                        ...
                        <Amount
                            sats={this.state.adjustedDestinationBalance || 0}
                            sensitive
                        />
                    </View>
                </View>
