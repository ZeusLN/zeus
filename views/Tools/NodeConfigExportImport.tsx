...
> {
    ...
    private executeExport = async () => {
        const { selectedNodes, useEncryption, exportPassword } = this.state;
        const { SettingsStore } = this.props;
        const { settings } = SettingsStore;
        ...
        try {
            ...
            const nodes = settings?.nodes || [];
            const selectedNodeConfigs = selectedNodes.map(
                (index) => nodes[index]
            );
            const exportString = createExportData(
                selectedNodeConfigs,
                useEncryption,
                exportPassword
            );
            ...
            await saveNodeConfigExportFile(filename, exportString);
    ...
    private handlePasswordSubmit = async (password: string) => {
        ...
        try {
            this.setState({ activeModal: 'none', isLoading: true });
            const content = await RNFS.readFile(importFilePath!, 'utf8');

            const nodes = decryptExportData(content, password);
