...
> {
    state = {
        processing: true,
        currentStep: 'Processing shared image...'
    };
    ...
    async componentDidMount() {
        ...
        if (requiresAuth) {
            this.setState({ currentStep: 'Authentication required...' });
        ...
        if (requiresWalletSelection) {
            this.setState({ currentStep: 'Wallet selection required...' });
    ...
    async processQRCode(qrData?: string, base64Image?: string) {
        ...
        try {
            this.setState({ currentStep: 'Extracting QR code from image...' });
            ...
            if (finalQrData) {
                this.setState({ currentStep: 'Processing payment data...' });
