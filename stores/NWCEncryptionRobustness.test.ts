...
describe('NWC Encryption Robustness Tests', () => {
    ...
    beforeEach(() => {
        ...
        cryptoProvider = {
            ...
            encryptNip44: (plaintext: string, key: string): string | Error => {
                ...
                return `nip44:${Buffer.from(plaintext).toString(
                    'hex'
                )}:${key.slice(0, 16)}`;
            },
            ...
            encryptNip04: (
                ...
                // Mock encryption
                return `nip04:${Buffer.from(plaintext).toString(
                    'hex'
                )}:${pubkey.slice(0, 16)}`;
            },
    ...
    describe('NIP-44 Key Validation', () => {
        ...
        it('should reject null and undefined keys', () => {
            ...
            expect(cryptoProvider.isValidNip44Key(undefined as any)).toBe(
                false
            );
        });
        ...
        it('should validate keys in tag structures', () => {
            ...
            expect(cryptoProvider.isValidNip44Key(invalidTag.value)).toBe(
                false
            );
        });
    });
    ...
    describe('Malformed Tag Handling', () => {
        it('should reject tags with missing encryption method', () => {
            const malformedTag: { name: string; value?: string } = {
                name: 'missing_method'
                // No value
            };

            expect(cryptoProvider.isValidNip44Key(malformedTag.value)).toBe(
                false
            );
        });
    ...
    describe('Fallback Mechanisms', () => {
        it('should fallback to NIP-04 when NIP-44 encryption fails', () => {
            ...
            const nip04Key =
                'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'; // Valid

            const nip44Result = cryptoProvider.encryptNip44(
                plaintext,
                nip44Key
            );
            expect(nip44Result).toBeInstanceOf(Error); // NIP-44 fails

            const nip04Result = cryptoProvider.encryptNip04(
                plaintext,
                nip04Key
            );
        ...
        it('should try NIP-44 first, then fallback gracefully', () => {
            ...
            let encrypted = cryptoProvider.encryptNip44(
                plaintext,
                invalidNip44Key
            );
            ...
            if (encrypted instanceof Error) {
                encrypted = cryptoProvider.encryptNip04(
                    plaintext,
                    validNip04Key
                );
            }
        ...
        it('should not fallback if fallback key is also invalid', () => {
            ...
            let result = cryptoProvider.encryptNip44(
                plaintext,
                invalidNip44Key
            );
            expect(result).toBeInstanceOf(Error);
            ...
            if (result instanceof Error) {
                result = cryptoProvider.encryptNip04(
                    plaintext,
                    invalidNip04Key
                );
            }
    ...
    describe('Decryption Robustness', () => {
        ...
        it('should not crash on decryption of non-hex data', () => {
            ...
            const invalidCiphertext =
                'nip44:zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz:keyprefix'; // Invalid hex
    ...
    describe('End-to-End Encryption Flows', () => {
        ...
        it('should handle complete NIP-04 encryption/decryption flow', () => {
            ...
            const decrypted = cryptoProvider.decryptNip04(
                encrypted as string,
                pubkey
            );
            expect(decrypted).toBe(plaintext);
