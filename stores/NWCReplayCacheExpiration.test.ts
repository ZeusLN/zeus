...
describe('NWC Replay Cache Expiration Boundary Tests', () => {
  ...
  describe('Expiration Boundary Conditions', () => {
    ...
    it('rejects replay marker at exactly 24 hours', () => {
      const cacheCreatedAt = currentTime;
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-3', status: 'processed' };
    ...
    it('handles nanosecond-precision timestamps near boundary', () => {
      ...
      const cacheEntry = { created: preciseCacheCreatedAt, eventId: 'test-4', status: 'processed' };

      // Move to 23h 59m 59.999s from creation
      currentTime = Math.floor(preciseCacheCreatedAt + REPLAY_CACHE_DURATION_MS - 1);
  ...
  describe('Clock Skew Scenarios', () => {
    it('handles forward clock skew (system time jumps ahead)', () => {
      // Normal cache entry
      const cacheCreatedAt = currentTime;
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-5', status: 'processed' };
    ...
    it('handles backward clock skew (system time jumps behind)', () => {
      ...
      const cacheCreatedAt = currentTime;
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-6', status: 'processed' };
    ...
    it('handles multiple clock skew events during cache lifetime', () => {
      const cacheCreatedAt = currentTime;
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-7', status: 'processed' };

      // Skew 1: Jump ahead 10 hours
      currentTime = cacheCreatedAt + (10 * 60 * 60 * 1000);
      let isExpired = currentTime - cacheEntry.created >= REPLAY_CACHE_DURATION_MS;
      ...
      isExpired = currentTime - cacheEntry.created >= REPLAY_CACHE_DURATION_MS;
    ...
    it('detects invalid clock state (creation time in future)', () => {
      ...
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-8', status: 'processed' };
  ...
  describe('Batch Cache Expiration', () => {
    it('expires multiple entries correctly at different times', () => {
      const entries = [
        { created: currentTime - (23 * 60 * 60 * 1000), eventId: 'batch-1' }, // 23h old
        { created: currentTime - (23.5 * 60 * 60 * 1000), eventId: 'batch-2' }, // 23.5h old
        { created: currentTime - (24 * 60 * 60 * 1000), eventId: 'batch-3' }, // 24h old
        { created: currentTime - (24.5 * 60 * 60 * 1000), eventId: 'batch-4' }, // 24.5h old
      ];

      const expiredEntries = entries.filter(
        (entry) => currentTime - entry.created >= REPLAY_CACHE_DURATION_MS
      );

      // Should expire entries 3 and 4 (24h and 24.5h old)
      expect(expiredEntries.length).toBe(2);
      expect(expiredEntries.map((e) => e.eventId)).toEqual(['batch-3', 'batch-4']);
    });

    it('preserves non-expired entries during batch purge', () => {
      const entries = [
        { created: currentTime - (1 * 60 * 60 * 1000), eventId: 'keep-1' }, // 1h old
        { created: currentTime - (12 * 60 * 60 * 1000), eventId: 'keep-2' }, // 12h old
        { created: currentTime - (23 * 60 * 60 * 1000), eventId: 'keep-3' }, // 23h old
      ];
      ...
      expect(nonExpiredEntries.map((e) => e.eventId)).toEqual(['keep-1', 'keep-2', 'keep-3']);
    });
  });
  ...
  describe('Boundary Edge Cases', () => {
    it('handles zero-duration expiration (immediate expiry)', () => {
      const cacheCreatedAt = currentTime;
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-9', status: 'processed' };
    ...
    it('handles very old cache entries (days beyond expiry)', () => {
      ...
      const cacheEntry = { created: cacheCreatedAt, eventId: 'test-10', status: 'processed' };
    ...
    it('correctly computes remaining TTL before expiry', () => {
      ...
      const checkTTL = (timeOffset: number) => {
        const checkTime = cacheCreatedAt + timeOffset;
  ...
  describe('Concurrent Request Handling Near Boundary', () => {
    it('handles concurrent requests at 23h 59m 59s boundary', async () => {
      const cacheCreatedAt = currentTime;
      const requests = [
        { id: 'req-1', timestamp: cacheCreatedAt + (23 * 60 * 60 * 1000 + 59 * 60 * 1000 + 59 * 1000) },
        { id: 'req-2', timestamp: cacheCreatedAt + (23 * 60 * 60 * 1000 + 59 * 60 * 1000 + 59 * 1000 + 500) },
        { id: 'req-3', timestamp: cacheCreatedAt + (23 * 60 * 60 * 1000 + 59 * 60 * 1000 + 59 * 1000 + 999) },
      ];

      const processRequest = (req: any) => {
        currentTime = req.timestamp;
        const isExpired = currentTime - cacheCreatedAt >= REPLAY_CACHE_DURATION_MS;
