...
export const AUTH_STORAGE_STRING = 'zeuspay-auth';
export const AUTH_DATE_STRING = 'zeuspay-auth-date';
...
export default class LightningAddressStore {
    ...
    private getAuthData = async () => {
        const lastSavedDateStored = await Storage.getItem(AUTH_DATE_STRING);
        const lastSavedDate = lastSavedDateStored
            ? new Date(Number(lastSavedDateStored))
            : undefined;

        const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000);

        if (lastSavedDate && lastSavedDate > tenMinutesAgo) {
            const lastSavedAuthStored = await Storage.getItem(
                AUTH_STORAGE_STRING
            );
            const auth = JSON.parse(lastSavedAuthStored);
            return auth;
        } else {
            ...
            const auth = { verification, signature };

            await Storage.setItem(AUTH_STORAGE_STRING, auth);
            await Storage.setItem(AUTH_DATE_STRING, Date.now());
