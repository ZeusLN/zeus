...
describe('NWC Storage Persistence Failure Tests', () => {
    ...
    return {
      setItem: jest.fn((key: string, value: string) => {
        if (storageErrorsEnabled) {
          throw new Error('Storage.setItem failed: Quota exceeded or permission denied');
        }
  ...
  describe('Replay Marker Persistence', () => {
    it('should not mark event processed if replay marker persistence fails', async () => {
      ...
      const saveReplayMarker = (eventId: string) => {
        try {
          storage.setItem(`replay:${eventId}`, JSON.stringify({ eventId, timestamp: Date.now() }));
          eventMarked = true;
        } catch (error) {
          throw new Error(`Failed to persist replay marker: ${error}`);
        }
      };
      ...
      eventMarked = false;
      expect(() => saveReplayMarker('event-2')).toThrow('Failed to persist replay marker');
    ...
    it('should prevent concurrent replays when marker persists', async () => {
      ...
      const processReplayCheck = (eventId: string): boolean => {
        ...
        storage.setItem(`replay:${eventId}`, JSON.stringify({ timestamp: Date.now() }));
    ...
    it('should fail gracefully if replay marker cannot be persisted', async () => {
      ...
      const saveReplayMarkerWithErrorHandling = (eventId: string): boolean => {
        try {
          storage.setItem(`replay:${eventId}`, JSON.stringify({ eventId, timestamp: Date.now() }));
          return true;
        } catch (error) {
      ...
      expect(persistenceError?.message).toContain('Storage.setItem failed');
    });
  });
  ...
  describe('Response Cache Persistence', () => {
    it('should not update response cache if persistence fails', async () => {
      ...
      const saveResponseToCache = (eventId: string, response: any) => {
        try {
          ...
          throw new Error(`Failed to persist response cache: ${error}`);
        }
      };
      ...
      expect(() => saveResponseToCache('event-2', response2)).toThrow('Failed to persist response cache');
    ...
    it('should block handler execution if response cache cannot persist', async () => {
      ...
      const executeAndCacheHandler = async (eventId: string): Promise<any> => {
        const response = { result: 'success', data: 'payload' };

        try {
          // Attempt to cache response before considering success
          storage.setItem(`response:${eventId}`, JSON.stringify(response));
          ...
          cachedResult = null;
          throw new Error(`Handler execution blocked: response cache persistence failed`);
        }
      };
    ...
    it('should not mutate event state if persistence fails', async () => {
      ...
      const markEventAsProcessed = (event: any) => {
        try {
          ...
          storage.setItem(`event:${event.eventId}`, JSON.stringify({ ...event, status: 'processed', processedAt: Date.now() }));
    ...
    it('should prevent partial state mutations on cascading persistence failures', async () => {
      ...
      const processEventTransaction = (eventId: string) => {
        try {
          ...
          storage.setItem(`response:${eventId}`, JSON.stringify({ eventId }));
  ...
  describe('Error Propagation', () => {
    it('should propagate storage errors to handler', async () => {
      ...
      expect(handlerError?.message).toContain('Storage.setItem failed');
    });
    ...
    it('should include error context in propagated exceptions', async () => {
      ...
      const saveWithContext = (eventId: string, context: any) => {
        try {
          storage.setItem(`event:${eventId}`, JSON.stringify({ eventId, ...context }));
        } catch (error) {
          throw new Error(
      ...
      try {
        saveWithContext('evt-123', { method: 'pay_invoice', amount: 5000 });
      } catch (error) {
  ...
  describe('Recovery and Retry', () => {
    it('should succeed on retry after transient storage failure', async () => {
      ...
      const persistWithRetry = (key: string, value: any, maxRetries = 3): boolean => {
    ...
    it('should give up after max retries', async () => {
      ...
      const persistWithLimitedRetry = (key: string, value: any, maxRetries = 3): boolean => {
        for (let i = 0; i < maxRetries; i++) {
          try {
            attemptCount++;
            storage.setItem(key, JSON.stringify(value));
      ...
      storageErrorsEnabled = true;
      const success = persistWithLimitedRetry('fail-key', { data: 'test' }, 3);
