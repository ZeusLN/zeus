...
export default class LightningNodeConnect {
    ...
    private calls = new Set<Promise<any>>();
    ...
    private async performBackendCall<T>(action: () => Promise<T>): Promise<T> {
        const resultPromise = action();
        this.calls.add(resultPromise);
        return resultPromise
            .then((response: any) => {
                if (this.calls.delete(resultPromise)) {
                    return response;
                }
    ...
    connect = () => this.performBackendCall(() => this.lnc.connect());
    checkPerms = () =>
        this.performBackendCall(async () => {
    ...
    isConnected = () => this.performBackendCall(() => this.lnc.isConnected());
    disconnect = () => this.lnc?.disconnect();

    getTransactions = () =>
        this.performBackendCall(() =>
            this.lnc.lnd.lightning
                .getTransactions({})
    ...
    getBlockchainBalance = (req: lnrpc.WalletBalanceRequest) =>
        this.performBackendCall(() =>
            this.lnc.lnd.lightning
                .walletBalance(req)
    ...
    openChannelStream = (data: OpenChannelRequest) =>
        this.performBackendCall(() => {
            let request: any = {
                ...
                sat_per_vbyte: !data.funding_shim
                    ? data.sat_per_vbyte
                    : undefined,
                spend_unconfirmed: data.spend_unconfirmed,
                ...
                params = {
                    ...
                    ...(this.supportInboundFees() && {
                        inboundFee: {
                            ...
                            fee_rate_ppm: `${
                                Number(data.fee_rate_inbound) * 10000
                            }`
                        }
                    }),
        ...
        this.performBackendCall(() => {
            const req: any = {
                ...
                startTime: Math.round(
                    new Date(Date.now() - hours * 60 * 60 * 1000).getTime() /
                        1000
                ).toString(),
    ...
    getUTXOs = (req: walletrpc.ListUnspentRequest) =>
        this.performBackendCall(() =>
            this.lnc.lnd.walletKit
                .listUnspent(req)
                .then((data: walletrpc.ListUnspentResponse) => snakeize(data))
        );
    bumpFee = (req: walletrpc.BumpFeeRequest) =>
        this.performBackendCall(() =>
            this.lnc.lnd.walletKit
                .bumpFee(snakeize(req))
                .then((data: walletrpc.BumpFeeResponse) => snakeize(data))
        );
    bumpForceCloseFee = (req: walletrpc.BumpForceCloseFeeRequest) =>
        this.performBackendCall(() =>
            this.lnc.lnd.walletKit
    ...
    importAccount = (req: walletrpc.ImportAccountRequest) =>
        this.performBackendCall(() =>
            this.lnc.lnd.walletKit
                .importAccount(req)
