...
**Final chapter (2026-08, KEY-004 remediation):** the saga's actual root cause turned out to be an upstream bug, not iCloud semantics: react-native-keychain's `cloudSyncValue()` treats a PRESENT `cloudSync: false` as true (`@NO` is a non-nil, truthy ObjC pointer), so every "exclude from iCloud" attempt above, and all `zeus:*` writes since, actually wrote `kSecAttrSynchronizable = true`. All wallet seeds were syncing to iCloud Keychain the entire time. Remediation (branch `fix/keychain-cloudsync-key004`): (1) `patches/patch-keychain-cloudsync.mjs` fixes the check and throws at postinstall if it stops matching; (2) `keychainDesyncMigration` (one-shot, non-destructive, runs FIRST in `getSettings`) copies synchronizable `zeus:*` items to the now-truly-local partition; (3) deletion happens via the one-shot, preflight-verified `autoPurgeLegacyKeychain` (`utils/KeychainPurgeUtils.ts`), fired from Wallet boot after the desync flag proves local copies exist; Tools > Keychain Cleanup remains as the manual fallback. The purge was originally consent-gated behind a one-time dialog; the maintainer reversed that in 2026-09 (PR #4385) to automatic deletion, accepting the #3625 propagation mode deliberately: the sync copies are the KEY-004 exposure itself, iCloud sync was never an advertised feature, and node-bound wallets (embedded LND, LDK Node) never survived device transfer anyway. The residual accepted risk is a device still on a pre-desync build whose only copy is the synced entry. The "never delete in a migration" lesson stands: the purge is not a migration, runs only after desync completes, and never deletes an only-copy.
