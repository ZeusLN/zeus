...
- TLS: cert verification is ON by default for new remote node configs (`trusty: !certVerification` in blob-util); trust-all is an explicit per-node opt-out confirmed via warning modal. If the node has `pinnedCerts` (from lndconnect `cert=` / clnrest `certs=`), a patched react-native-blob-util enforces byte-exact cert pinning, which takes precedence over both CA validation and trust-all. Tor requests bypass TLS validation only for HTTPS `.onion` URLs (see zeus-failure-archaeology FA-5 for the incident that set this rule). WebSocket streaming endpoints bypass Tor entirely.
