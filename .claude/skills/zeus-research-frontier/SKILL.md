...
| TLS verification now ON by default for new configs (PR #4366) | `@observable certVerification: boolean = true` in `stores/SettingsStore.ts`; connection-string cert material drives per-node pinning (`pinnedCerts`, patched react-native-blob-util) | `grep -n "certVerification: boolean = " stores/SettingsStore.ts` |
...
3. **`certVerification` default-flip campaign (new-config side shipped in PR #4366):** new configs default to `true` with the warning modal on opt-out, and lndconnect/clnrest cert material now drives cert pinning. Nodes saved before the flip are normalized to an explicit stored `false` by the `cert-verification-default-v1` MOD_KEY migration. REMAINING: migrating that trust-all cohort to verified/pinned TLS needs maintainer sign-off and a deliberate UX path (re-scan to pin, or opt-in verification) — route through **zeus-storage-and-migrations** + **zeus-change-control**. Also open: WebSocket streams (`wsReq`) have no pinning path and still require CA-valid certs.
